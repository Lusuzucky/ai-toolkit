...
def run_cache_te(config_path_or_dict: Union[str, dict, OrderedDict], device: Optional[str] = None):
    ...
    # Force only Text Encoder loading
    model_config.only_load_te = True
    if "te_dtype" not in model_raw:
        model_config.te_dtype = train_config.dtype
    if not hasattr(model_config, "model_kwargs") or model_config.model_kwargs is None:
        model_config.model_kwargs = {}
    model_config.model_kwargs["only_load_te"] = True
    ...
    print_acc(f"[Cache TE] Initializing {ModelClass.__name__} for Text-Encoder-only caching on {run_device} ({dtype})...")
    model = ModelClass(
        device=run_device,
    ...
    for raw_ds in datasets_raw:
        ...
        for s in split:
            s["cache_text_embeddings"] = True
            s["cache_latents_to_disk"] = False
            s["cache_latents"] = False
            dataset_configs.append(DatasetConfig(**s))

    print_acc(f"[Cache TE] Starting text embeddings caching for {len(dataset_configs)} dataset(s)...")
    for i, ds_cfg in enumerate(dataset_configs):
        print_acc(f"[Cache TE] Dataset {i + 1}/{len(dataset_configs)}: {ds_cfg.folder_path}")
        dataset = AiToolkitDataset(ds_cfg, sd=model)
        flush()

    print_acc("[Cache TE] All text embeddings successfully cached to disk!")


def main():
    parser = argparse.ArgumentParser(description="Standalone Text Encoder caching tool - caches prompt embeddings to disk without loading Transformer or VAE.")
    parser.add_argument("--config", "-c", type=str, required=True, help="Path to job yaml config")
    parser.add_argument("--device", "-d", type=str, default=None, help="Device to run TE on (default: accelerator device)")

    args = parser.parse_args()
    run_cache_te(args.config, device=args.device)
